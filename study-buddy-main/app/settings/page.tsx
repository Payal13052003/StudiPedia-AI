...
export default function SettingsPage() {
  ...
  // Define the settings key in one place for easy management
  const SETTINGS_KEY = "StudiPedia AI-settings";
  ...
  const loadSettings = async () => {
    try {
      ...
      // Fallback in case of API error or JSON parsing issues
      const savedSettings = localStorage.getItem(SETTINGS_KEY);
      if (savedSettings) {
  ...
  const handleSave = async () => {
    try {
      // Save to localStorage for persistence using the new key
  ...
  const handleReset = () => {
    // Remove the item using the new key

...
// Documentation content has been updated to "StudiPedia AI"
const docsContent = {
  overview: {
    title: "Documentation Overview",
    content: `
StudiPedia AI Documentation

Welcome to StudiPedia AI's comprehensive documentation! Everything you need to know about using and configuring your AI personal tutor.
...
- Visit our GitHub repository for source code

About This Documentation

This documentation is available both within the app and on GitHub for your convenience. All information is kept up-to-date with the latest version of StudiPedia AI.

StudiPedia AI is an open-source AI personal tutor that runs locally on your computer, providing personalized learning experiences while maintaining complete privacy.
    `
  },
...
Learn how to use StudiPedia AI effectively for your learning goals.
...
StudiPedia AI is designed with privacy as a core principle:
...
StudiPedia AI provides comprehensive AI-powered tutoring with flexible configuration options.
...
Configure StudiPedia AI to work best for your needs and preferences.
...
Quick solutions for common issues with StudiPedia AI.
...
3.   Restart App  : Close and reopen StudiPedia AI
...
-   Antivirus Blocking  : Add StudiPedia AI to exclusions
...
2.   Delete Config Folder  :
   - Windows: %APPDATA%/studipedia-ai/
   - macOS: ~/Library/Application Support/studipedia-ai/
   - Linux: ~/.config/studipedia-ai/
...
3.   About Page  : Version and feature information
    ...
    Community Support
1.   GitHub Issues  : Report bugs on our repository
2.   Discussions  : Ask questions on our repository
3.   Feature Requests  : Suggest improvements on our repository

    Reporting Issues
Include these details:
- Operating system and version
- StudiPedia AI version (from About page)
...
Speed up your StudiPedia AI experience with these keyboard shortcuts.
...
Remember: Keyboard shortcuts make StudiPedia AI more accessible and efficient for all users!
    `
  },
...
Understanding StudiPedia AI's architecture and technical implementation.

   Architecture Overview

StudiPedia AI is built as a hybrid desktop application using:
...
StudiPedia AI uses Server-Sent Events (SSE) for real-time AI responses:
...
-   Health Checks  : Service availability verification
   ...
   Extensibility
    ...
    Plugin Architecture
StudiPedia AI is designed for future extensibility:
...
Quick development setup and workflow information for StudiPedia AI.
...
Complete guide for generating StudiPedia AI's graduation cap icons.
...
The fastest way is to use https://favicon.io/emoji-favicons/graduation-cap/ and download the complete package, then organize the files according to the structure above.
  ...
  Note  : StudiPedia AI already has generated graduation cap icons! This guide is for reference if you need to regenerate or customize them.
    `
  },
...
Complete reference for StudiPedia AI's backend API endpoints.
...
Content-Type: application/json
X-StudiPedia-AI-Settings: string (optional JSON-encoded settings)
...
fetch('/api/getChat', {
  ...
  headers: {
    'Content-Type': 'application/json',
    'X-StudiPedia-AI-Settings': settings
  },
...
While StudiPedia AI uses SSE, WebSocket implementation would look like:
...
---
 ...
 For implementation details, see the source code on our GitHub repository. 
    `
  }
};
...
export default function DocsPage() {
  ...
  const renderContent = (content: string) => {
    // This is a basic parser. For a real app, a library like `react-markdown` would be better.
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('```')) {
            if (inCodeBlock) {
                elements.push(
                    <pre key={`code-${i}`} className="bg-gray-800 text-white p-4 rounded-md my-4 overflow-x-auto">
        ...
        if (line.startsWith('  ')) {
            elements.push(<h1 key={i} className="text-3xl font-bold text-gray-900 mt-8 mb-4 border-b pb-2 first:mt-0">{line.substring(2)}</h1>);
        } else if (line.startsWith('   ')) {
            elements.push(<h2 key={i} className="text-2xl font-semibold text-gray-900 mt-6 mb-3 border-b pb-2">{line.substring(3)}</h2>);
        } else if (line.startsWith('    ')) {
            elements.push(<h3 key={i} className="text-xl font-medium text-gray-900 mt-5 mb-2">{line.substring(4)}</h3>);
        } else if (line.startsWith('|')) {
            ...
            if (separator && separator.includes('---')) {
                        ...
                        <table className="min-w-full border border-gray-300">
                            <thead>
                                <tr className="bg-gray-100">
                                    {headers.map((header, index) => <th key={index} className="px-4 py-2 text-left font-semibold text-gray-700 border-b">{header}</th>)}
                                </tr>
                            </thead>
                            <tbody>
                                {rows.map((row, rowIndex) => (
                                    <tr key={rowIndex} className="border-b">
                                        {row.map((cell, cellIndex) => <td key={cellIndex} className="px-4 py-2 text-gray-700">{cell}</td>)}
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                i = j - 1; // Move index past the table
            }
        } else if (line.startsWith('- ')) {
            elements.push(<li key={i} className="text-gray-700 ml-6 list-disc">{line.substring(2)}</li>);
        } else if (line.trim()) {
            // Handle inline formatting like bold and links
            const parts = line.match(/(`[^`]*`)|(\[[^\]]+\]\([^\)]+\))|([^\`\[]+)/g) || [];
            const renderedLine = parts.map((part, index) => {
                if (typeof part !== 'string') return null; // Ensure only strings are processed
                if (part.startsWith('`') && part.endsWith('`')) {
                    return <code key={index} className="bg-gray-200 text-gray-800 px-1 py-0.5 rounded-md font-mono text-sm">{part.slice(1, -1)}</code>;
                }
                const linkMatch = typeof part === 'string' ? part.match(/\[([^\]]+)\]\(([^)]+)\)/) : null;
                if (linkMatch) {
                    return <a key={index} href={linkMatch[2]} className="text-blue-600 hover:text-blue-800 underline" target="_blank" rel="noopener noreferrer">{linkMatch[1]}</a>;
                }
                return part;
            }).filter(Boolean); // Remove any nulls
            elements.push(<p key={i} className="text-gray-700 my-2 leading-relaxed">{renderedLine}</p>);
        } else {
            // Render empty lines as small gaps
